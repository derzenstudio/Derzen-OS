...
export interface TenantMeta {
  ...
  /** Seeded sample workspace. Only these appear in the login page demo list,
   *  and only these take the plaintext demo-password sign-in path. */
  isDemo: boolean;
  /** Epoch ms until which billing is waived. Undefined means normal billing. */
  freeUntil?: number;
...
export function ensureRuntimeTenant(c: CustomerAccount): TenantMeta {
  ...
  if (!t) {
    t = {
      ...
      created: new Date(c.createdAt).toISOString().slice(0, 10), features: {}, isDemo: false,
    };
    TENANTS.push(t);
  }
  return t;
