...
// Total lookup: an id that is no longer in the registry (restored slice,
// archived unit, stale deep link) must not throw on a property read.
...
// A reservation can outlive its guest record: reservations are persisted per
// tenant, GUESTS was not, so a restored chat booking used to resolve to
// undefined and white-screen every view that read `.name`. The lookup is now
// total. It self-heals by registering a placeholder, so downstream writers
// (invoices, CSV export, conversations) still find a record on the next read.
const placeholderGuest = (id: string): Guest => ({
  id, name: "Unknown guest", emails: [], phones: [], country: "Unknown",
  status: "active", lastActivityTs: Date.now(), lastSource: "web", lifetimeSpend: 0,
  tags: ["unresolved"], notes: "Record missing at load; recreated as a placeholder.",
  consentMarketing: false, aliases: [], verifiedId: false,
});
export const guestById = (id: string): Guest => {
  const found = GUESTS.find((x) => x.id === id);
  if (found) return found;
  const stub = placeholderGuest(id);
  GUESTS.push(stub);
  return stub;
};
...
const REPLACE: [string, unknown[]][] = [
  ["properties", PROPERTIES], ["guests", GUESTS], ["conversations", CONVERSATIONS], ["reservations", RESERVATIONS],
